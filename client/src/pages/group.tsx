...
export default function GroupPage() {
  ...
  const getProductCount = (collectionId: string) =>
    products.filter(
      (p) => p.collectionId === collectionId && p.isActive !== false,
    ).length;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="container mx-auto px-6 py-16">
        {subgroups.length === 0 ? (
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-sm">
              As categorias de <strong>{group.name}</strong> aparecerão aqui em
              breve.
            </p>
          </div>
        ) : (
          <>
            ...
            >
              {subgroups.map((sub, i) => {
                ...
                const promo = getCollectionPromo(
                  activePromotions,
                  sub.id,
                  groupId!,
                );
                const cashbackPct: number | null =
                  (sub as any).cashbackPct ?? null;
                return (
                  <motion.div
                    key={sub.id}
                      ...
                      <div className="group relative overflow-hidden rounded-2xl cursor-pointer shadow-md hover:shadow-xl transition-all duration-300">
                        ...
                        >
                          ...
                          <div
                            ...
                            style={{
                              background:
                                "linear-gradient(to top, rgba(0,0,0,0.9) 0%, rgba(0,0,0,0.2) 60%)",
                            }}
                          />
      ...
      <footer className="py-10 text-center border-t border-gray-100">
        ...
        <Link href="/shop">
          <span
            className="text-xs uppercase tracking-widest cursor-pointer hover:opacity-70 transition-opacity"
            style={{ color: "#000000" }}
          >
